...
Ncrit = [9, 7.3, 2.4];
vacc = [0.01, 0.023, 0.0075];
vacc_2 = [0.0075, 0.0075, 0.0075, 0.0085, 0.008];%for the final piece with the low chord
% starting the loops:
for i = 1:1:no_elements
    for j = 1:1:5
    ...
    C_l(i) = C_l(i,end);
    C_d(i) = C_d(i,end);
    d_L(i) = 0.5*rho*V_R(i)*V_R(i)*c(i)*C_l(i)*del_r(i);
    d_D(i) = 0.5*rho*V_R(i)*V_R(i)*c(i)*C_d(i)*del_r(i);
end
for i = 1:1:no_elements
