...
mach = [0.01, 0.5, 0.1, 0.12, 0.15, 0.18, 0.20, 0.21, 0.3];
...
for i = 1:1:length(power_settings)
    figure(i)
    for j = 1:1:length(altitudes)
        plot(mach,thrust_pounds_feet_per_second(:,j,i),'LineWidth',2);hold on
        title(strcat(num2str(power_settings(i)),' percent power setting thrust map'))
    end
    xlabel('Mach');ylabel('Thrust (lb_f)');grid on
    leg1 = legend(num2str(altitudes'));
    title(leg1,'Altitude (ft)')
end
