...
% SHAFT POWER MAP
[power_settings, RPMs, power_to_propeller_w_losses, Fuel_flow_w_alt] =...
...
for i = 1:1:length(altitudes)
    ...
    figure(figure_no)
    for j=1:1:length(power_settings)
        plot(RPMs, Fuel_flow_w_alt(j, :, i),'LineWidth',2);hold on
        title(strcat('Altitude =  ',num2str(altitudes(i)),' ft'))
    end
    xlabel('RPM');ylabel('Fuel Flow (lb/hr)');grid on
    leg1 = legend(num2str(power_settings'), 'Location','NorthWest');
    title(leg1,'power settings')
    ylim([0 20])
end
