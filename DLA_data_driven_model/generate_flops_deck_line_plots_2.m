...
altitudes = [0,4000,8000,12000, 13000, 16000];
...
fprintf(fid_engine, "%4s    %10s    %5s   %5s   %10s   %10s      %10s      %10s\n", ...
    " Mach No.", "Altitude", "Power Setting", "Thrust", "Power", "Ram Drag", "Fuel Flow", "RPM");
for i = 1 : length(mach)
    m = mach(i);
    for j = 1 : length(altitudes)
        alt = altitudes(j);
        thrust_corresponding_ps = thrust_pounds_feet_per_second(i, j, :);
        power_corresponding_ps = Power_Extracted_3D_Matrix(i, j, :);
        for k = 1 : length(power_settings)
            ps = power_settings(k);
            thrust = thrust_corresponding_ps(k);
            power = power_corresponding_ps(k);
            fuel_flow = fuel_consumption_lb_per_hr(k, j);
            rpm = RPM_PS(k);
            
            fprintf(fid, "%5.3f%10.3f%5d%10.3f%10.3f%10.3f%10.3f\n", m, alt, ps, thrust, power, 0, fuel_flow);
            fprintf(fid_engine, " %5.3f      %10.3f      %5d      %10.3f	%10.3f      %10.3f      %10.3f      %10.3f\n", m, alt, ps, thrust, power, 0, fuel_flow, rpm);
...
for i = 1:1:length(altitudes)
    ...
    leg2 = legend(num2str(power_settings'), 'location','NorthWest');
    ylim([0 11]);
