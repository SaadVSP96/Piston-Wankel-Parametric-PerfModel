%we define power settings, the rpms of TOA data and then follow it by the
%extended rpm array for which the deck is to be generated
...
for i = 1:1:length(Power_TOA(:,1))  
    SFC_TPS(i,:) =  interp1(RPMs_TOA,SFC_TOA(i,:),RPMs,'linear','extrap');%g/kWh
end
figure(2)
for i = 1:1:length(SFC_TPS(:,1))
    plot(RPMs, (SFC_TPS(i,:).*0.0016439868), 'LineWidth',1.5);hold on
end
